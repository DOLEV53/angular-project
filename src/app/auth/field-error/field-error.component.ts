...
export class FieldErrorComponent {
   @Input() formField?: FormControl<any>;
    ...
    fieldErr(): string {
        ...
        if (maxlengthErr) {
            return `The number of chars cannot be longer than ${maxlengthErr.requiredLength}`;
        }

        const minlengthErr = control.getError('minlength');
        if (minlengthErr) {
            return `the number of chars cannot be shorter than ${minlengthErr.requiredLength}`;
        }
